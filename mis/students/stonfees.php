...
<div class="main-content" id="mainContent">
    <div class="stfees">
        ...
        // SQL query to fetch fees data for the logged-in student from fees table
                $select1 = "SELECT s.program, f.selct, f.pname, f.paname, f.amount, f.date, f.status 
                FROM students s
                INNER JOIN fees f ON s.stid = f.student_id
                WHERE s.stid = ? AND f.pname IS NOT NULL
                ORDER BY f.date DESC";  // Added ordering by date
                ...
                if (!$stmt1->execute()) {
                die("<p style='color: red;'>Execute failed: " . $stmt1->error . "</p>");
                }

                $res1 = $stmt1->get_result();
                if (!$res1) {
                die("<p style='color: red;'>Get result failed: " . $stmt1->error . "</p>");
                }
