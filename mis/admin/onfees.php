...
if (isset($_POST['submit'])) {
    ...
    $misc_text = isset($_POST['misc']) ? 'Miscellaneous' : ''; // Set as Miscellaneous if checkbox is checked

    try {
        $con->begin_transaction();
        
        // Get all student IDs for the selected program
        $studentQuery = $con->prepare("SELECT stid FROM students WHERE program = ?");
        ...
        // Update balances ONLY if miscellaneous is checked (year is optional)
        if (isset($_POST['misc'])) {
            // If year is provided, update only students with matching year
            if (!empty($year)) {
                $updateStmt = $con->prepare("UPDATE students SET balance = balance + ? WHERE program = ? AND level = ?");
                $updateStmt->bind_param("iss", $amount, $program, $year);
            } 
            // If year is not provided, update all students in the program
            else {
                $updateStmt = $con->prepare("UPDATE students SET balance = balance + ? WHERE program = ?");
                $updateStmt->bind_param("is", $amount, $program);
            }
            $updateStmt->execute();
        }
            ...
            <?php
             if (isset($_POST['select'])) {
                ...
                $display2 = "
                    ...
                    FROM students
                    LEFT JOIN fees ON students.stid = fees.student_id AND students.level = fees.year
                    WHERE students.program = '" . $con->real_escape_string($choose2) . "'
                ";
    ...
    <div id="myModal3" class="modal3">
     <div class="modal-content3">
        ...
        <form class="form" method="post">
            ...
            <input type="hidden" name="allst" value="">
           ...
           <label>Amount:</label>
           <input type="number" id="fee-amount" name="amount" required oninput="updateRadioValue()">
           <select name="year">
           ...
           <input type="radio" id="misc-radio" name="misc" value="1" style="margin-left:-340px;">
